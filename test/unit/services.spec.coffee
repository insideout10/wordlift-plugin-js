...
#            dump "[ entity-annotation id :: #{entityAnnotationId} ][ entity id :: #{entityAnnotation.entity.id} ][ confidence :: #{entityAnnotation.confidence} ]"
  ...
  describe 'EditorService', ->
    ...
    it 'finds entities using the itemid attribute', inject( (AnalysisService, EditorService) ->

      html = ''
      results = ''

      # Get the html against which the analysis has been run.
      $.ajax('base/app/assets/insideout10.html', { async: false } ).done (data) ->
        html = data

      # Get the mock-up analysis results.
      $.ajax('base/app/assets/insideout10.json', { async: false } ).done (data) ->
        results = data

      existingEntities = EditorService.findEntities html

      # Parse and merge the analysis results.
      analysis = AnalysisService.parse results, true

      # Check that the analysis results exist.
      expect(analysis).toEqual jasmine.any(Object)
      ...
      findEntityAnnotation = (entityAnnotations, uri) ->
        for id, entityAnnotation of entityAnnotations
          return entityAnnotation if uri is entityAnnotation.entity.id or uri in entityAnnotation.entity.sameAs
        null

        # Find the existing entities in the html
      for match in existingEntities
        ...
        if textAnnotation
          ...
          dump "match [ id :: #{textAnnotation.id} ][ start :: #{textAnnotation.start} ][ end :: #{textAnnotation.end} ][ entity annotation :: #{entityAnnotation.id} ]"
        else
          dump "no match [ start :: #{match.text.start} ][ end :: #{match.text.end} ][ label :: #{match.label} ]"

      for id, textAnnotation of analysis.textAnnotations
        start = textAnnotation.start
        end = textAnnotation.end
        text = textAnnotation.selectedText
        dump "textAnnotation [ start :: #{start} ][ end :: #{end} ][ text :: #{text} ]"
